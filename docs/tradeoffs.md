...
LLM scoring and email generation are real in OpenAI/OpenRouter modes because model reasoning is central to the demo. A fake provider exists for tests and no-key local development only.
